...
class DatasetProvider:
    ...
    def get_image(self, **kwargs) -> np.ndarray:
        """
    ...
    def get_next_image_batch(self, **kwargs) -> List[np.ndarray]:
        """
    ...
    def _resize_image(image: np.ndarray, target_size=(224, 224)) -> np.ndarray:
    ...
    def _random_rotation(image: np.ndarray) -> np.ndarray:
    ...
    def _random_zoom(image: np.ndarray) -> np.ndarray:
        h, w = image.shape[:2]
        ...
        if new_h < h or new_w < w:
            ...
            cropped = image[:h, :w]
        return DatasetProvider._resize_image(cropped)
    
    def _set_black_background(image: np.ndarray, threshold=20) -> np.ndarray:

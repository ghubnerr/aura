...
mod tests {
    ...
    {
        let url_str = "ws://127.0.0.1:3030/signaling";
    ...
    #[tokio::test]
    async fn test_custom_server_creation() {
        let server = SignalingServer::new(Some(8080), Some("0.0.0.0".to_string()));
    ...
    async fn test_client_connection() {
        let server = SignalingServer::new(None, None);
        Python::with_gil(|py| {
            server.start(py).unwrap();
        });

        // Give server time to start
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;

        // Use the URL string directly instead of url::Url
        let (_ws_stream, _) = tokio_tungstenite::connect_async("ws://127.0.0.1:3030/signaling")
            .await
            .expect("Failed to connect");

        Python::with_gil(|py| {
            assert_eq!(server.get_client_count(py).unwrap(), 1);
        });
    }
    ...
    async fn test_message_broadcast() {
        ...
        let message = serde_json::to_string(&offer).unwrap();
        ...
        if let Some(Ok(TungsteniteMessage::Text(received))) = client2.next().await {
            let parsed: SignalingMessage = serde_json::from_str(&received).unwrap();
            match parsed {
